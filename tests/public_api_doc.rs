...
/// Published library crates in this workspace. `zenpredict-viz` is
/// `publish = false`; `zenpicker-train` is excluded from the workspace;
/// `zentrain` is Python.
const CRATES: &[&str] = &["zenanalyze", "zenpredict", "zenpicker", "zenpredict-bake"];
...
fn public_api_surface_docs_are_current() {
    ...
    for package in CRATES {
        ...
        if *package == "zenanalyze" {
            // Surface-freeze enforcement artifact: per CLAUDE.md ("API
            // stability", user clarification 2026-05-17) breaking changes
            // are batched into a 0.x minor bump; within a 0.x line the
            // surface only grows.
            doc.push_str(
                "# POLICY: zenanalyze's public API is tightly governed (see CLAUDE.md) —\n\
                 # additions only within a 0.x line; breaks need a minor bump + changelog.\n",
            );
        }
