...
| `train_hybrid.py` | **Recommended.** Hybrid-heads training: N categorical cells + K scalar prediction heads per cell. Codec supplies the cell taxonomy via `parse_config_name`. `--activation` picks the **trainer**, not just the activation: `leakyrelu` (the default) is the PyTorch student (seconds-to-minutes; Kaiming init, no L2 unless `--weight-decay`), `relu` is sklearn's single-threaded `MLPRegressor.fit` (10–20× slower at the same shape; Glorot init, always `alpha=1e-4` L2). [#68](https://github.com/imazen/zenanalyze/issues/68) originally measured sklearn relu at argmin_acc 43.0% vs leakyrelu 30.6% (zenwebp v0.2, n=1, pre-#67 per-head normalization); the 2026-08-28 re-run on the canonical zenwebp dataset with n=3 seeds (`benchmarks/train_hybrid_backend_gap_zenwebp_2026-08-28.md`) measures relu 11.2±1.2% / leakyrelu 9.1±0.6% / `leakyrelu --weight-decay 1e-4` 12.3±1.0% argmin_acc at mean overhead 4.82 / 4.71 / 4.58% (zenjpeg canonical n=1: 24.6 / 21.7 / 25.2%, mean 12.04 / 12.71 / 12.37%) — the gap does not reproduce and weight decay closes what remains, so `--weight-decay` now defaults to 1e-4 for the torch student (pass `0.0` for the old unregularized bake). Compare trainers on your own data with `leakyrelu_seeds_runner.py` before a production bake; every bake records its trainer in `safety_report.diagnostics.student_backend`. Other flags: `--objective {size_optimal, zensim_strict, rd_time, time_budgeted}` (the time-aware pair, [#56](https://github.com/imazen/zenanalyze/issues/56) / [#43](https://github.com/imazen/zenanalyze/issues/43), require the Pareto's `encode_ms` column and train the per-cell `time_log` head under `--time-loss-weight`; `time_budgeted` also requires `--time-budget-multiplier > 0`), `--bytes-quantile`, `--reach-threshold`, `--hidden 192,192,192`, `--out-suffix` |
...
| `canonical_to_pareto.py` | Canonical per-codec dataset (`s3://zentrain/canonical/<date>/<codec>/{train,validate,test}.parquet`) → the PARETO parquet + FEATURES TSV `train_hybrid.py` consumes: `config_name` = the `cell` label, `size_class` re-derived from `width×height` (the canonical column is `large` for every row), named `feat_*` only (positional zensim `feat_N` dropped), one features row per image (verified constant within the image). Pass every split — train_hybrid re-derives the origin even/odd split itself. Config: `examples/canonical_picker_config.py` (`ZENCANONICAL_CODEC=zenwebp|zenjpeg|…`, `ZENCANONICAL_DIR`) — one categorical `mode` axis over the canonical cells |
| `fit_family_degradation.py` | zenanalyze#85: per-family **effort-cap degradation** (`ln bytes_at(cap) − ln bytes_at(reference effort)` per image × target, aggregated per size_class) and **per-effort encode cost** (`ms/MP` p50/p90 + `ln ms = a + b·ln px` fit) from a canonical dataset. Knob semantics per codec (`m`/`e` ascending, rav1e `s` inverted). Committed tables: `benchmarks/family_degradation_{zenwebp,zenjxl,zenavif}_2026-08-28.{tsv,md}`; the API decision they feed is `docs/meta-picker-degradation-2026-08-28.md` |
| `leakyrelu_seeds_runner.py` | zenanalyze#68: multi-seed, multi-arm student-trainer comparison (`relu` sklearn / `leakyrelu` torch / `leakyrelu+wd=1e-4`) on one codec config — identical data, split, hidden shape; aggregates val argmin diagnostics `mean ± stdev` and echoes each arm's `student_backend` record. `--reuse` picks up existing model JSONs. Report: `benchmarks/train_hybrid_backend_gap_zenwebp_2026-08-28.md` |
...
| `--objective {size_optimal, zensim_strict, rd_time, time_budgeted}` | 2 | Default `size_optimal`. Bake `zensim_strict` alongside for SLA traffic; `rd_time` / `time_budgeted` for compute-bound callers (time head REQUIRED; see #56 / #43) |
| `--time-loss-weight 0.5` | 2 | α in `bytes_loss + α · time_loss` for the student (rd_time / time_budgeted default 0.5; 0 drops the time block) |
| `--time-budget-multiplier 1.5` | 2 | Label-time budget filter (required > 0 under `time_budgeted`); drives `BUDGET_INFEASIBLE` |
