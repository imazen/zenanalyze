...
| `train_hybrid.py` | **Recommended.** Hybrid-heads training: N categorical cells + K scalar prediction heads per cell. Codec supplies the cell taxonomy via `parse_config_name`. `--activation` picks the **trainer**, not just the activation: `leakyrelu` (the default) is the PyTorch student (seconds-to-minutes; Kaiming init, no L2 unless `--weight-decay`), `relu` is sklearn's single-threaded `MLPRegressor.fit` (10–20× slower at the same shape; Glorot init, always `alpha=1e-4` L2). [#68](https://github.com/imazen/zenanalyze/issues/68) measured sklearn relu at argmin_acc 43.0% vs leakyrelu 30.6% on identical zenwebp v0.2 data (n=1, pre-#67 per-head normalization) — use leakyrelu for capacity sweeps / ablation and confirm a production bake against relu (or `leakyrelu --weight-decay 1e-4`) until that gap is re-measured; every bake records its trainer in `safety_report.diagnostics.student_backend`. Other flags: `--objective {size_optimal, zensim_strict}`, `--bytes-quantile`, `--reach-threshold`, `--hidden 192,192,192`, `--out-suffix` |
