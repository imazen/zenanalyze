...
PARETO = Path("/home/lilith/work/zen/zenjpeg--ablate-overnight-2026-05-02/benchmarks/zenjpeg_pareto_strat100_2026-05-02.tsv")
FEATURES = Path("/home/lilith/work/zen/zenjpeg--ablate-overnight-2026-05-02/benchmarks/zenjpeg_pareto_features_strat100_2026-05-02.tsv")

# Where to write the trained model + summary:
OUT_JSON = Path("/home/lilith/work/zen/zenjpeg--ablate-overnight-2026-05-02/benchmarks/zenjpeg_overnight_hybrid_2026-05-02.json")
OUT_LOG = Path("/home/lilith/work/zen/zenjpeg--ablate-overnight-2026-05-02/benchmarks/zenjpeg_overnight_hybrid_2026-05-02.log")
...
# Overnight sweep used the simpler zq_pareto_calibrate harness which
# only varies color × sub × progressive × auto_optimize. Map those
# names to the trellis/chroma_scale schema by collapsing to noT_cs100
# (no trellis, default chroma scale) and treating progressive/sa as
# the SA bit (closest equivalent).
_OVERNIGHT_RE = re.compile(
    r"^(?P<color>ycbcr|xyb)_(?P<sub>444|420)_"
    r"(?P<mode>baseline|progressive|auto_optimize)$"
)
...
def parse_config_name(name: str) -> dict:
    ...
    """
    m_o = _OVERNIGHT_RE.match(name)
    if m_o:
        return {
            "color": m_o.group("color"),
            "sub": m_o.group("sub"),
            "sa": m_o.group("mode") == "auto_optimize",
            "trellis_on": False,
            "lambda": LAMBDA_NOTRELLIS_SENTINEL,
            "chroma_scale": 1.0,
        }
