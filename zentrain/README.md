# zentrain — picker training pipeline (Python)

Pareto sweep harness, teacher fit, distill, ablation, holdout probes, safety reports, and the bake step that produces ZNPR v2 `.bin` artifacts. Both per-codec config pickers and the codec-family meta-picker train through this same pipeline; only the `cells` axis differs.

The runtime that loads those `.bin`s is [`zenpredict`](../zenpredict/); the meta-picker layer is [`zenpicker`](../zenpicker/) (renamed from the unpublished Rust shell — what used to live here under `src/`).

AGPL-3.0-only / Commercial dual license.
...
| File | What it is |
|---|---|
| **[../zenpredict/examples/hybrid_heads_codec_sketch.rs](../zenpredict/examples/hybrid_heads_codec_sketch.rs)** | Runnable codec-side reference: `CELLS` table, constraint mask, `argmin_masked_in_range` + scalar clamp |
| **[../zenpredict/examples/load_baked_model.rs](../zenpredict/examples/load_baked_model.rs)** | Smoke test for the loader on any bake artifact |
...
| **[../tools/bake_picker.py](../tools/bake_picker.py)** | sklearn JSON → ZNPR v2 binary (Python emits `BakeRequestJson`, `zenpredict-bake` produces the `.bin`). Codec-agnostic |
| **[../tools/bake_roundtrip_check.py](../tools/bake_roundtrip_check.py)** | Verify Rust loader matches numpy reference forward pass |
| **[../tools/test_bake_roundtrip.py](../tools/test_bake_roundtrip.py)** | End-to-end regression: 3 activations × 3 dtypes round-trip against synthetic models |
...
```
                ...
                └──────────┬──────────┘
                           ↓
                ┌─────────────────────┐
                │  Train teacher      │  ← zentrain/tools/train_hybrid.py
                ...
                └──────────┬──────────┘
                           ↓ JSON
                ┌─────────────────────┐
                │  Bake to ZNPR v2  │  ← tools/bake_picker.py
...
The training pipeline lives at [`zentrain/tools/`](tools/README.md) — codec-agnostic. Each codec writes a small **codec config module** declaring its TSV paths, feature subset, target_zq grid, and config-name parser; the training scripts import it via `--codec-config <module-name>`.
...
Emits the v2 binary blob (~30 KB f16 / ~60 KB f32) plus `models/zenjpeg_picker_v1.manifest.json` describing the per-output config metadata. Both ship in the codec crate via `include_bytes!`.
...
let mut picker = zenpredict::Predictor::new(model);
...
// codec converts to an allow mask via `zenpredict::threshold_mask`,
...
The reach-safety table is shipped in **both** profiles' manifests (size_optimal carries it for transparency / debugging; the codec just doesn't apply the gate under that profile). The training script (`tools/train_hybrid.py --reach-threshold 0.99`) records both the raw `reach_rate[c]` per zq and a precomputed `safe[c]` boolean at the bake-time threshold. **Codec consumers should prefer the runtime helper** (`zenpredict::threshold_mask(reach_rates, threshold_at_request_time, out)`) so the threshold is a *parameter* — at very high target_zq (≥ 96), the strict 0.99 gate leaves 0–2 cells safe; "max-quality mode" callers pass `threshold = 0.0` and rely on the rescue path for tail safety. The bake-time `safe[]` is convenient when the codec wants the strict threshold without re-evaluating, nothing more.
...
```rust,ignore
let adjust = zenpredict::ArgminOffsets {
...
| ✅ v0.1 | **Rescue plumbing** (`zenpredict::{should_rescue, RescuePolicy, RescueStrategy, RescueDecision}`) — codec-agnostic decision logic; codec injects `verify` + `rescue_pick`. See [SAFETY_PLANE.md](SAFETY_PLANE.md) |
| ✅ v0.1 | **Reach-gate as parameter** (`zenpredict::threshold_mask`) — codec / caller picks the threshold per request. Strict 0.99 = SLA mode, 0.95 = high-q, 0.0 = max-quality (relies on rescue) |
...
For the per-codec burden to stay low, the picker-side framework lives in `zenpredict::rescue` (decision logic + threshold predicate + strategy/decision enums) and `Predictor::argmin_masked_top_k` (cached second-best). Encode and verify themselves stay codec-side because zenpicker has no decoder, no zensim, no jpegli/lambda awareness — but the codec only writes the orchestration glue, not the rules. Sketch:

```rust
use zenpredict::{should_rescue, RescuePolicy, RescueStrategy, RescueDecision, AllowedMask};
