...
| ~~`tier_depth` EOTF~~ **DONE** | PQ/HLG EOTF per channel | per sampled pixel (HDR) | **row-batched `linear_srgb::default::*_to_linear_slice`** — linear-srgb's *public* auto-dispatched SIMD EOTF (magetypes f32x16 poly; zentone uses it too). Gather each row's samples → SIMD EOTF slice → per-pixel stats. Measured **10.84×** vs scalar (and 2.8× the LUT — a hand-rolled u16 LUT was a first, wrong attempt; the slice wins at the 100k budget). Handles all transfers + channel types; ~few-ULP vs the scalar f64-intermediate → **within tolerance, no re-bless**; deterministic on FMA arches. |
