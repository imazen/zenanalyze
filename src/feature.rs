...
// before the rest of the dispatcher runs, so the per-feature gating
// inside `analyze_specialized_raw` skips the corresponding work
// without touching tier internals. Crate-internal — the dispatch
// plan's narrowing logic is not part of the public surface (callers
// just see `None` for features they didn't explicitly drop, and the
// analyzer chose to skip).

/// Features that depend on chroma signal and are dropped by the
/// dispatch plan when the strict-grayscale classifier reports
/// `is_grayscale = true`. On a true grayscale image these features
/// either compute to ~0 (mean-of-zero gradients) or are
/// definitionally zero (skin tones can't survive grayscaling).
/// Skipping them on grayscale input avoids paying for Tier 2 + the
/// chroma half of Tier 3 + the Tier 1 chroma accumulators.
///
/// Includes the Tier 1 chroma signals (`ChromaComplexity`,
/// `CbSharpness`, `CrSharpness`, `Colourfulness`), every Tier 2 axis,
/// chroma DCT compressibility, the UV noise / quant percentile
/// columns, and `SkinToneFraction`.
#[allow(unused_mut, unused_assignments)]
...
/// near-identical blocks) or to the floor of their AQ-map range —
/// in both cases the consumer learns nothing the cheap `uniformity`
/// signal didn't already say.
///
/// `AqMapMean` / `AqMapStd` are kept (they're cheap byproducts of
/// the same accumulator that produces the percentiles); only the
/// percentile columns get dropped.
#[allow(unused_mut, unused_assignments)]
