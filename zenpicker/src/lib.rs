...
impl<'b> MetaPicker<'b> {
    ...
    pub fn validate_family_order(&mut self) -> Result<(), MetaPickerError> {
        ...
        let parsed = parse_family_csv(raw).ok_or_else(|| MetaPickerError::Metadata(
            format!("unparseable family_order CSV: {:?}", raw),
        ))?;
...
mod tests {
    ...
    fn time_cost_table_uses_codec_family_index() {
        ...
        let reindexed: alloc::vec::Vec<f32> =
            bake_order.iter().map(|f| table[f.index()]).collect();
